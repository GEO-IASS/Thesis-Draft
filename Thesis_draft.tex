...
In the spatial frequency content, fractal scaling is expressed as a power law. Extracting fractal characteristic form Consistent with previous work (XXXXX), our scans for a given slip surface display fixed scaling across out instrumental array (see figure XXX). The fixed scaling provides means to remove instrumental artefacts. For our instruments, artefacts are particularly disruptive at the small scale instrumental limit. The LiDar and laser scanner are dominated by random noise, a shallower slope at the high frequency tail of the spectra. Conversely, the spectra of the scan collected using the white light interferometer have steeper high frequency tails. By comparing individual scan to a manually identified scaling across all our instrumental scales of observation, we identify a conservatively defined bandwith overwhich the data is valid. We reject data associated to affected frequencies for subsequent analysis.

We can further distill the roughness metric using the fractal model of the fault:

\begin{equation}
P(k) = Ck^{-\beta}
\end{equation}

Accordingly the entire spectral information of a scan can be summarized with the prefactor ($C$) and the scaling exponent ($\beta$). The fractal model is parametrized using a power law regression through the spectra
