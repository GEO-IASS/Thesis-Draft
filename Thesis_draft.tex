...
Where $\beta$, $n$, and $K$ are constants. 
	...
	Running-in stage was proposed to reflects the effect of surface roughness. Specifically, the failure of interlocking asperities of initially rough surface is thought to explain the higher wear-rates in the running-in stage. The formulation was supported by a semi-analytical model for the failure asperities with simple hemispherical geometry  and rotary shear experiments. 
	
	In line with comments in \citet(power1988roughness, wang1994wear}, we propose that the transient stage of wear on real faults is likely to be substantially different for faults. The fractal topography of slip surfaces implies that progressively larger asperities will come into contact as displacement increases. In contrast,  all experiments to-date have been based on saw-cut surfaces that are nominally planar. We highlight that wear processes controlling the behaviour laboratory scale experiments are likely inadequate for natural faults--these may instead reflect behaviour issuing from limited fractal bandwidth and emplacement of mobile gouge layers buffering the surfaces. These processes may be active on natural surfaces. However, we question whether or not theses are the \textit{dominant} processes that define the geometry of natural fault slip surface 
