...
In this section, I present the work flow for processing scan data. We use spatial statistic to characterize and infer processes active on faults--specifically  on the slip surface. To do so the following work flow is implemented \textit{MatLab}:
...
Before conducting a statistical analysis scan data must be pre-processed into a workable format. Scan data has the form of point cloud data - a series of points with coordinates $x$, $y$ and $z$. Scans obtained in this study typically yielded on the order of $10^6$ to $ 10^7$ points. In raw form, the point clouds are randomly oriented, noisy and still contain artefacts of defects on the scanned surface. All these features must be removed. The process of manual removal is labour intensive and infeasible for a data set of the scope presented in this study. I rather opt to automate this process. Only very large defects (defects that may substantially effect the quality of finding a true mean plane) are manually removed.

		\subsubsection{Point Cloud orientation}

In order to yield proper spatial statistics, the surface must both be flattened and aligned along slip. Linear trends in the data induce unwanted signal in the scan data and effect the quality of later interpolation onto a grid. Thus, $x$, $y$, $z$ data is rotated using around the axis $\textbf{u}$ as determined by the normalized cross product between the surface normal $\textbf{n}$ and a vector normal to a flat surface ($\textbf{n}_2 = [0 0 1]$): 
...
At bandwidths larger than the microscopic scale, fault surfaces are anisotropic. The anisotropy is associated to the direction of slip. The the 'smoothest' direction is slip parallel; the 'roughness' direction is perpendicular to slip. In this study, we narrow down our analysis to these two most extreme cases. As such the fault scan is rotated to be aligned with the direction of slip. The direction of slip is detected to within a degree by determining the smoothest direction by spectral analysis. This process is streamlined by decimating raw data iterating steps of regridding, rotating and spectral analysis. For the purpose of this analysis, roughness is paramterized as the integral of the log-weighted power spectrum over the entire bandwidth of the sample surface. The angle associated to the minimum in roughness is then identified as the direction of slip and, sub-sequentially, used to rotated the entire flattened data cloud. The data is then interpolated onto a grid using a linear interpolation algorithm.
...
Defects are filtered out of the original surface using multiple thresholding methods. The basis of these methods all revolve around identifying clear points or segments that are statistical outliers to the distribution characterizing the entire surface--abnormal points. 

The most aggressive filter implemented is the fractal model outlier. The filter removes entire segments with abnormally high variance in the height field for the given scale of observation. For this implementation, I iterate over 10 scales. The scales are selected using a log spacing up to the size of the entire surface. The threshold for segment removal is chosen to be four standard deviations from the mean. Note that the filter should not be associated to a systematic bias. Segment removal is symmetric. As a result, given a truly stationary fault surfaces (as assumed in the subsequent analysis) the removal of extreme values should not systematically affect the data. Outlier associated with a truly distinct genetic mechanism (in this case cracks and other defects) \textit{will} be removed.

Abnormally flat sections arise as the result of triangular interpolation of sparse points. Typically, interpolation on the edge on a non-convex set of points has this effect. These artefacts are removed by removing abnormally low curvatures in the log-transformed absolute value curvature field. The filter rejects point with curvatures less than $10^{-25}$. These values are unrealistically small and can only be associated to the linear interpolation scheme.
