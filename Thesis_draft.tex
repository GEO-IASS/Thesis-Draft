...
We separate our scan data into two distinct populations. 1) Scan data with direct displacement constraint and 2) scan data associated to maximum displacement constrains. We hypothesize that faults smooth as a function of displacement. In this case as done in Brodsky et al., 2011, we test the validity of a power law fit to describe the —the smoothing model. Scan data with direct displacement constraints serves as a direct test of the smoothing model. For the model to be valid it must agree with further constraints imposed by data with only maximum displacement estimates. 

All spectra are plotted and color-coded according to their displacement constraints.
